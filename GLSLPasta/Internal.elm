    ...
    let
        ...
        f global (oldErrors, oldSymbols) =
            case insertGlobal part.id global oldSymbols of
                Ok newSymbols -> (oldErrors, newSymbols)
                Err newError -> (oldErrors ++ [newError], oldSymbols)
    in
...
combineGlobals parts =
    let
        ...
        f part (oldErrors, oldSymbols) =
            let
                (newErrors, newSymbols) = insertGlobals part oldSymbols
            in
                (oldErrors ++ newErrors, newSymbols)

        (errors, symbols) = List.foldl f ([], symbols0) parts
    in
        case errors of
...
combineFunctions parts =
    let
        ...
        functions = List.concatMap .functions parts
    in
        Ok (String.join "\n" functions)
...
combineSplices parts =
    let
        splices : List Splice
        splices = List.concatMap .splices parts
    in
        Ok (String.join "\n" splices)
